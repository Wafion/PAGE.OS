...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const results = rawLinks
      .filter((link) => link.href && /\.(pdf|txt)$/i.test(link.href))
      .map((link) => ({
        title: link.title,
        link: link.href!,
        type: link.href!.toLowerCase().endsWith('.pdf') ? 'pdf' as const : 'txt' as const,
      }));
    ...
    return NextResponse.json(
      { error: 'Failed to scrape Brave results', details: errorMessage },
      { status: 500 },
    );
  }
}
